...
async fn edit_ghost_menu(bot: &Bot, chat_id: ChatId, msg_id: MessageId, state: &AppState) -> ResponseResult<()> {
    ...
    let save_examples = ghost_state.map(|g| g.save_as_examples).unwrap_or(true);
    
    let text = format!(
        "👻 <b>Ghost Mode</b>\n\n\
        Статус: {}\n\
        Сохранение примеров: {}\n\n\
        В этом режиме ваши сообщения отправляются от имени бота.\n\
        Полезно для обучения персоны на примерах.",
        if is_active { "🟢 Активен" } else { "🔴 Выключен" },
        if is_active && save_examples { "✅" } else { "❌" }
    );
    
    let kb = InlineKeyboardMarkup::new(vec![
        vec![
            InlineKeyboardButton::callback("🟢 Вкл + сохр.", "ghost_on"),
            InlineKeyboardButton::callback("🟡 Вкл без сохр.", "ghost_on_nosave"),
        ],
...
<b>Пример промпта:</b>
<code>Ты — дружелюбный помощник по программированию. Отвечай кратко и по делу. Используй примеры кода когда уместно.</code>

<b>Советы:</b>
• Будьте конкретны в описании характера
• Укажите стиль речи (формальный/неформальный)
• Опишите области знаний
...
async fn edit_help_ghost(bot: &Bot, chat_id: ChatId, msg_id: MessageId) -> ResponseResult<()> {
    let text = r#"👻 <b>Ghost Mode</b>

Режим "призрака" позволяет владельцу отправлять сообщения от имени бота.

<b>Зачем нужен:</b>
• Обучение персоны на примерах
• Ручное управление диалогом
• Демонстрация возможностей

<b>Режимы:</b>
• <b>С сохранением</b> — ваши сообщения сохраняются как примеры для персоны
• <b>Без сохранения</b> — просто отправка от имени бота

<b>Как использовать:</b>
1. Включите Ghost Mode
2. Пишите сообщения как обычно
3. Они будут отправлены от имени бота
4. Выключите когда закончите

<b>Важно:</b>
• Работает только для владельца
• Ваши оригинальные сообщения удаляются
• Бот не отвечает автоматически в этом режиме"#;
